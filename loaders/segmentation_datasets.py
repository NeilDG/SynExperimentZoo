...
color_arrays = {color: np.array(color) for color in color_to_class}  # precompute colors.
...
#         color_mask = np.all(mask_img == color_arrays[color], axis=-1)
...
#     #     encoded_mask[color_mask] = class_id
#
#     encoded_mask = torch.from_numpy(encoded_mask).to(torch.int64)  # To tensor and Long type
#     return encoded_mask


def mask_to_labels(mask_img):
    """Encodes the mask to [0, 1, 2, 3] labels."""
    # mask_img = mask_img.permute(1, 2, 0)  # (H, W, 3)
    encoded_mask = torch.zeros((mask_img.shape[0], mask_img.shape[1]), dtype=torch.int64)  # (H, W)

    for color, class_id in color_to_class.items():
        label_tensor = torch.tensor(color, dtype=torch.uint8)
        color_mask = torch.all(mask_img == label_tensor, dim=-1)
        encoded_mask[color_mask] = class_id

    return encoded_mask

def labels_to_mask(mask_labels:torch.uint8):
