...
import model.modules.basicblock as B
import torch
...
class SRMD(nn.Module):
    def __init__(self, in_nc=19, out_nc=3, nc=128, nb=12, upscale=4, act_mode='R', upsample_mode='pixelshuffle'):
        ...
        if(upscale > 1):
            ...
            m_tail = B.conv(nc, out_nc, mode="CBR", bias=bias)
            self.model = B.sequential(m_head, *m_body, m_tail)
...
if __name__ == '__main__':
    model = SRMD(in_nc=3, out_nc=3, nc=64, nb=15, upscale=4, act_mode='R', upsample_mode='pixelshuffle')
    print(model)
